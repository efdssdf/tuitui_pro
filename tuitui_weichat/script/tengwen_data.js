...
const end = new Date(now_time).setSeconds(0,0);
const last_time = now_time - 60*1000;
const start = new Date(last_time).setSeconds(0,0);
...
async function mapOrderDataSource(dataSource) {
    for(let i = 0; i < dataSource.length; i ++) {
        ...
        await PlatformData.findOneAndUpdate({wx_gzhopenid}, {
            wx_userid: userid, 
            amount, 
            order_time: atime,
            ispay
